...
def build_control_group(midi_range='21-28', number=1, layout=LayoutAxis.row):
...
def build_controller_v2(groups=[build_control_group()]):
    return ControllerV2(
        on_led_midi =1,
        off_led_midi=1,
    ...
    def build_mode_model_v2(self):
        return DeviceV2(
            ...
            range_maps=[
                RowMapV2(
                    row=2,
                    range=RangeV2(from_=1, to=8),
                    parameters=RangeV2(from_=1, to=8)
                ),
                RowMapV2(
                    row=1,
                    range=RangeV2(from_=1, to=8),
                    parameters=RangeV2(from_=9, to=16)
                )
            ])
