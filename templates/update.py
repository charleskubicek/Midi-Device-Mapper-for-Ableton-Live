...
def main():
    ...
    parser.add_argument('command', type=str, choices=['reload', 'debug', 'hudtrace', 'dump', 'dump2', 'dumpnames', 'lom', 'rackprobe', 'doctor', 'showinfo', 'cs_dir', 'options'], help='Command to be executed')
    ...
    if args.command == 'cs_dir':
        ...
        message = b'lom'
        send_udp_message(message, ip, port)
    elif args.command == 'rackprobe': # one-shot rack macro layout probe -> logs
        message = b'rackprobe'
        send_udp_message(message, ip, port)
